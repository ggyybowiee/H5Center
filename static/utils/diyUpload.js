...
(function ($) {
  ...
  $.fn.extend({
    ...
    diyUpload: function (opt, serverCallBack) {
      ...
      //迭代出默认配置
      $.each(getOption('#' + $fileInputId, opt), function (key, value) {
      ...
      webUploader.on('uploadError', function (file, reason) {
        var $fileBox = $('#fileBox_' + file.id);
        var $diyBar = $fileBox.find('.diyBar');
        showDiyProgress(0, $diyBar, 'error!');
        var err = 'upload err! file:' + file.name + ' errorCode:' + reason;
        if (errorCallBack) {
          // errorCallBack(err);
          errorCallBack('上传失败!');
        }
      ...
      webUploader.on('error', function (code) {
        ...
        switch (code) {
          ...
          case 'Q_EXCEED_NUM_LIMIT':
            text = '上传文件数量不能超过' + opt.thumb.fileNumLimit;
            break;
          case 'F_EXCEED_SIZE':
            text = '文件大小超过限制!';
            break;
  ...
  function getOption(objId, opt) {
    ...
    return {
      ...
      //配置生成缩略图的选项
      thumb: {
        width: opt.thumb.width || 160,
        height: opt.thumb.height || 120,
        // 图片质量，只有type为`image/jpeg`的时候才有效。
        quality: opt.thumb.quality || 100,
        // 是否允许放大，如果想要生成小图的时候不失真，此选项应该设置为false.
        allowMagnify: opt.thumb.quality || false,
        // 是否允许裁剪。
        crop: true,
        // 为空的话则保留原有图片格式。
        // 否则强制转换成指定的类型。
        type: opt.thumb.type || 'image/jpeg',
      },
      //文件上传方式
      ...
      fileNumLimit: opt.thumb.fileNumLimit || 5,
  ...
  //创建文件操作div;
  function createBox($fileInput, file, webUploader, successCallBack) {
    var file_id = file.id;
    var $parentFileBox = $fileInput.parents(".upload-ul");
    var file_len = $parentFileBox.children(".diyUploadHover").length;
    ...
    var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
					<div class="viewThumb">\
					    ...
					    <p class="diyControl"><span class="diyCancel"><i></i></p>\
					</div> \
				</li>';
    ...
    var $diyCancel = $fileBox.find('.diyCancel').one('click', function () {
      ...
      if (successCallBack) {
        successCallBack($parentFileBox.children(".diyUploadHover").length);
      }
    });
